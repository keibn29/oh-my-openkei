...
describe('orchestrator agent', () => {
  ...
  test('orchestrator prompt includes mandatory skill-loading for frontend-developer', () => {
    const agents = createAgents();
    const orchestrator = agents.find((a) => a.name === 'orchestrator');
    const prompt = orchestrator?.config.prompt as string;

    expect(prompt).toContain('### Mandatory Skill Loading');
    expect(prompt).toContain('@frontend-developer');
    expect(prompt).toContain(
      'Load all available skills using the skill tool before doing any work',
    );
  });

  test('orchestrator prompt includes mandatory skill-loading for backend-developer', () => {
    const agents = createAgents();
    const orchestrator = agents.find((a) => a.name === 'orchestrator');
    const prompt = orchestrator?.config.prompt as string;
  ...
  test('orchestrator prompt mandatory skill-loading does not mention agents outside the list', () => {
    const agents = createAgents();
    const orchestrator = agents.find((a) => a.name === 'orchestrator');
    const prompt = orchestrator?.config.prompt as string;

    // Agents like explorer that are not in SUBAGENT_MUST_LOAD_SKILLS_FIRST should not appear
    // in the Mandatory Skill Loading section (may appear elsewhere in the prompt).
    // trigger-developer was removed from the list so it should also not appear.
    const sectionMatch = prompt.match(
      /### Mandatory Skill Loading[\s\S]*?(?=### |$)/,
    );
    expect(sectionMatch).not.toBeNull();
    const section = sectionMatch?.[0] ?? '';
    expect(section).not.toMatch(/@explorer\b/);
    expect(section).not.toMatch(/@trigger-developer\b/);
    expect(section).not.toMatch(/@oracle\b/);
    expect(section).not.toMatch(/@librarian\b/);
  });
});
...
describe('skill permissions', () => {
  ...
  test('business-analyst gets wildcard allow like other primary agents', () => {
    const agents = createAgents();
...
describe('developer agent skills in prompt', () => {
  ...
  test('business-analyst prompt requires saving analysis to .business-analyst/', () => {
    const agents = createAgents();
    const ba = agents.find((a) => a.name === 'business-analyst');
    const prompt = ba?.config.prompt as string;

    // Must always save to file (not conditional like planner)
    expect(prompt).toContain('MUST ALWAYS save');
    expect(prompt).toContain('.business-analyst/');
    expect(prompt).toContain('Write tool');
    // Concise confirmation after save
    expect(prompt).toContain(
      'Do NOT repeat the full analysis in the chat message',
    );
    // Must not skip the file save
    expect(prompt).toContain('Skip the file save');
  });

  test('business-analyst custom prompt preserves mandatory output rules', () => {
    // customPrompt replaces the base prompt — but the output rules must survive.
    const ba = createBusinessAnalystAgent(
      'test/model',
      'You are a custom business analyst prompt.',
    );
    const prompt = ba.config.prompt as string;
...
describe('permission profiles', () => {
  ...
  test.each(
    READ_ONLY_NAMES,
  )("read-only agent '%s' has wildcard deny ('*': 'deny') at config level", (name) => {
  ...
  test.each(
    READ_ONLY_NAMES,
  )("read-only agent '%s' has read-only tools allowed", (name) => {
    const agents = createAgents({ disabled_agents: [] });
    const agent = agents.find((a) => a.name === name);
    expect(agent).toBeDefined();
    const permission = agent?.config.permission as Record<string, unknown>;
  ...
  test.each(
    READ_ONLY_NAMES,
  )("read-only agent '%s' does not have edit explicitly set (covered by '*')", (name) => {
    const agents = createAgents({ disabled_agents: [] });
    const agent = agents.find((a) => a.name === name);
    expect(agent).toBeDefined();
    const permission = agent?.config.permission as Record<string, unknown>;
  ...
  test.each(
    CAN_EDIT_NAMES,
  )("edit-capable agent '%s' does not have wildcard deny", (name) => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === name);
    expect(agent).toBeDefined();
    const permission = agent?.config.permission as Record<string, unknown>;
...
describe('agent classification', () => {
  test('SUBAGENT_NAMES excludes orchestrator and sprinter', () => {
    ...
    expect(SUBAGENT_NAMES).toContain('trigger-developer');
...
describe('question tool instruction in agent prompts', () => {
  const expectedText =
    'When you need to ask the user a question, you MUST use the `question` tool';

  test('orchestrator prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'orchestrator');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('planner prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'planner');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('sprinter prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'sprinter');
    expect(agent?.config.prompt).toContain(expectedText);
  });
  ...
  test('explorer prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'explorer');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('librarian prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'librarian');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('oracle prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'oracle');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('designer prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'designer');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('frontend-developer prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'frontend-developer');
    expect(agent?.config.prompt).toContain(expectedText);
  });

  test('backend-developer prompt includes question tool instruction', () => {
    const agents = createAgents();
  ...
  test('frontend-developer custom prompt still includes question tool instruction', () => {
    const fe = createFrontendDeveloperAgent(
      'test/model',
      'Custom frontend prompt',
    );
    expect(fe.config.prompt).toContain(expectedText);
  });

  test('backend-developer custom prompt still includes question tool instruction', () => {
    const be = createBackendDeveloperAgent(
      'test/model',
      'Custom backend prompt',
    );
    expect(be.config.prompt).toContain(expectedText);
  });

  test('custom agent definition includes question tool instruction', () => {
    // Verify custom agents built via PluginConfig get the instruction
    const config: PluginConfig = {
      agents: {
        myCustomAgent: { model: 'openai/gpt-4' },
      },
    };
    const agents = createAgents(config);
    const custom = agents.find((a) => a.name === 'myCustomAgent');
    expect(custom?.config.prompt).toContain(expectedText);
  });
});
...
describe('trigger-developer agent', () => {
  test('trigger-developer is created and in agents array', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'trigger-developer');
    expect(agent).toBeDefined();
  });

  test('trigger-developer uses same default model as other execution specialists', () => {
    const agents = createAgents();
    const triggerDev = agents.find((a) => a.name === 'trigger-developer');
    expect(triggerDev?.config.model).toBe(DEFAULT_MODELS['trigger-developer']);
  });

  test('trigger-developer is edit-capable without wildcard deny', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'trigger-developer');
    expect(agent).toBeDefined();
    const permission = agent?.config.permission as Record<string, unknown>;
  ...
  test('trigger-developer accepts model override', () => {
    ...
    const agents = createAgents(config);
    const agent = agents.find((a) => a.name === 'trigger-developer');
    expect(agent?.config.model).toBe('custom-trigger-model');
    expect(agent?.config.temperature).toBe(0.3);
  });

  test('trigger-developer is not disabled by default', () => {
    const agents = createAgents();
    const names = agents.map((a) => a.name);
    expect(names).toContain('trigger-developer');
  });

  test('trigger-developer prompt does NOT require loading available skills', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'trigger-developer');
    const prompt = agent?.config.prompt as string;
    expect(prompt).not.toContain('they are MANDATORY');
    expect(prompt).not.toContain(
      'use the `skill` tool to load all available skills',
    );
  });

  test('trigger-developer prompt includes question tool instruction', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'trigger-developer');
  ...
  test('trigger-developer gets trigger-related skills allowed by default', () => {
    const agents = createAgents();
    const agent = agents.find((a) => a.name === 'trigger-developer');
    const skillPerm = (agent?.config.permission as Record<string, unknown>)
      ?.skill as Record<string, string>;
    expect(skillPerm?.['trigger-setup']).toBe('allow');
    expect(skillPerm?.['trigger-tasks']).toBe('allow');
    expect(skillPerm?.['trigger-agents']).toBe('allow');
    expect(skillPerm?.['trigger-config']).toBe('allow');
    expect(skillPerm?.['trigger-realtime']).toBe('allow');
    expect(skillPerm?.['trigger-cost-savings']).toBe('allow');
    expect(skillPerm?.['karpathy-guidelines']).toBe('allow');
  });
});
